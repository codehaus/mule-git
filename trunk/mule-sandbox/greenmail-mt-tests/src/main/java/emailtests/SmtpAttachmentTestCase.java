...
/**
 * Smtp Attachment Test Case
 * 
 * @author Stephen Fenech
 * 
 * In this test we check sending attachments where the service component creates an EmailWithAttachmentMessage
 * which is then converted to a MimeMessage using a Transformer
 *
 */
public class SmtpAttachmentTestCase extends MailFunctionalTestCase{
	...
	public void testAttachment() throws Exception
	{
	    ...
	    for(int i=0;i<messageCount;i++)
	    {
	    	...
	    	//TODO check correctness of attachmentsString message=servers.util().getBody(messages[i]).trim();
