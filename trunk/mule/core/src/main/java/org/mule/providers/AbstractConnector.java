...
implements UMOConnector, ExceptionListener, UMOConnectable, WorkListener
{
	...
	public final synchronized void stopConnector() throws UMOException
	{
		...
		if (this.isConnected())
		{
			...
			{
				logger.error("Failed to disconnect, continuing (details at debug level): " + e.getMessage());
	...
	public final synchronized void dispose()
	{
		...
		{
			logger.warn("Failed to stop during shutdown, continuing (details at debug level): " + e.getMessage());
			logger.debug(e.getMessage(), e);
		}
	...
	protected void disposeReceivers()
	{
		if (receivers != null)
		{
			...
			for (Iterator iterator = receivers.values().iterator(); iterator.hasNext();)
			{
				...
				{
					logger.error("Failed to destroy receiver '" + receiver 
							+ "', continuing (details at debug level): " + e.getMessage());
					logger.debug(e.getMessage(), e);
				}
			}
	...
	protected void disposeDispatchers()
	{
		if (dispatchers != null)
		{
			...
			{
				logger.error("Failed to dispose dispatchers, continuing (details at debug level): " + ex.getMessage());
	...
	public UMOTransformer getDefaultInboundTransformer()
	{
		if (defaultInboundTransformer != null)
		{
			...
			{
				logger.error("Failed to clone default inbound transformer, continuing (details at debug level): " + e.getMessage());
				logger.debug(e.getMessage(), e);
			}
		}
		return null;
	...
	public UMOTransformer getDefaultResponseTransformer()
	{
		if (defaultResponseTransformer != null)
		{
			...
			{
				logger.error("Failed to clone default Outbound transformer, continuing (details at debug level): " + e.getMessage());
				logger.debug(e.getMessage(), e);
			}
		}
	...
	public UMOMessageAdapter getMessageAdapter(Object message) throws MessagingException
	{
		...
		catch (TransportServiceException e)
		{
			throw new MessagingException(
					new Message(Messages.FAILED_TO_CREATE_X, "Message Adapter"),
					message, e);
		}
	}
	...
	throws MessagingException
	{
		...
		{
			throw new MessagingException(
					new Message(Messages.FAILED_TO_CREATE_X, "Stream Message Adapter"),
					in, e);
		}
	}
