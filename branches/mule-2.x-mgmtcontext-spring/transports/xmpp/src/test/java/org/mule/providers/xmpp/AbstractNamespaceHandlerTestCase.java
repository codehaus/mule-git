...
public abstract class AbstractNamespaceHandlerTestCase extends FunctionalTestCase
{
    ...
    {
        XmppConnector connector = (XmppConnector) muleContext.getRegistry().lookupConnector(
    ...
    {
        UMOImmutableEndpoint simpleEndpoint = muleContext.getRegistry().lookupEndpointBuilder(
            "simpleEndpoint").buildOutboundEndpoint();
        assertEquals(protocolName + "://mule:secret@localhost:1234/recipient",
            simpleEndpoint.getEndpointURI().toString());

        UMOImmutableEndpoint groupChatEndpoint = muleContext.getRegistry().lookupEndpointBuilder(
            "groupChatEndpoint").buildOutboundEndpoint();
        assertEquals(protocolName + "://mule:secret@localhost:1234/recipient",
            groupChatEndpoint.getEndpointURI().toString());
